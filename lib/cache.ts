...
export class CacheService {
  ...
  // 缓存产品列表
  cacheProducts(locale: string, category: string, products: any[]): void {
    const key = `${CACHE_PREFIXES.PRODUCTS}${locale}:${category}`;
    this.set(key, products, 3600); // 缓存1小时
  }

  // 获取缓存的产列表
  getCachedProducts(locale: string, category: string): any[] | null {
    const key = `${CACHE_PREFIXES.PRODUCTS}${locale}:${category}`;
    return this.get<any[]>(key);
  }

  // 缓存单个产品
  cacheProduct(locale: string, productId: string, product: any): void {
    const key = `${CACHE_PREFIXES.PRODUCTS}${locale}:${productId}`;
    this.set(key, product, 1800); // 缓存30分钟
  }

  // 获取缓存的单个产品
  getCachedProduct(locale: string, productId: string): any | null {
    const key = `${CACHE_PREFIXES.PRODUCTS}${locale}:${productId}`;
    return this.get<any>(key);
  }
  ...
  // 缓存翻译数据
  cacheTranslations(locale: string, namespace: string, translations: any): void {
    const key = `${CACHE_PREFIXES.TRANSLATIONS}${locale}:${namespace}`;
    this.set(key, translations, 7200); // 缓存2小时
  }

  // 获取缓存的翻译数据
  getCachedTranslations(locale: string, namespace: string): any | null {
    const key = `${CACHE_PREFIXES.TRANSLATIONS}${locale}:${namespace}`;
    return this.get<any>(key);
  }
  ...
  invalidateProductCache(productId?: string): void {
    if (productId) {
      // 删除特定产品的所有语言缓存
      for (const locale of ['en', 'de', 'ja', 'fr', 'th', 'es', 'ru', 'pt', 'it', 'nl', 'pl', 'ko', 'id']) {
